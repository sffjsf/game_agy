...
export function executeClone(caster, skill, weaponSystem, effectSystem, dx, dy, dist) {
{
  ...
  for (var i = 0; i < caster.clones.length; i++) {
    weaponSystem.createRangedAttack(caster.clones[i].x, caster.clones[i].y, caster.target.x, caster.target.y, skill.damage, caster.team, 'shuriken', caster.charData.color, caster);
  }
  return;
}
